...
{
    class ClsPurchaseOrder : clsAddon
    {
        public const string Formtype = "142";
                ...
                else
                {
                    ...
                    switch (BusinessObjectInfo.EventType)
                    {
                        case SAPbouiCOM.BoEventTypes.et_FORM_DATA_LOAD:
                            ...
                            //aRInvoice.Auto_Mail("", "OPOR", "22");
                            break;
                        case SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD:
                        case SAPbouiCOM.BoEventTypes.et_FORM_DATA_UPDATE:
                            if (BusinessObjectInfo.ActionSuccess == false) return;
                            string strsql;
                            dbDataSource_Head = oForm.DataSources.DBDataSources.Item("OPOR");
                            strsql = clsModule.objaddon.objglobalmethods.getSingleValue("select 1 from OPOR where \"DocEntry\"=" + dbDataSource_Head.GetValue("DocEntry", 0) + " and \"CANCELED\" ='Y'");
                            if (strsql == "1") return;
                            aRInvoice.Auto_Mail(BusinessObjectInfo.FormUID,dbDataSource_Head.GetValue("DocEntry", 0), "OPOR", "22");
                            break;
